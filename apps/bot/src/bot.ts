...
import { EventQueue, type Event } from './eventQueue';
import { env } from './env';
...
const init = () => {
  return Effect.gen(function* () {
    ...
    if (
      state.sessionId === config.session.id &&
    ...
    yield* database.config.updateSessionStatus(isSessionValid ? "valid" : "invalid");
...
export const startBot = () => {
  return Effect.gen(function* () {
    ...
    yield* Effect.all([
      ...processors,
      Effect.repeat(init(), Schedule.addDelay(Schedule.forever, () => "1 second")),
