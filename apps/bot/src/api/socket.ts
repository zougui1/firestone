...
export const waitResponse = <T extends z.ZodSchema>(
  ...
  return Effect.async<z.output<T>, TimeoutError | ResponseError | Error>(resume => {
    ...
    const onMessage = (message: WebSocket.RawData) => {
      ...
      if (genericResponseResult.data.Function === 'UserOnMultipleInstances' && options?.kill !== false) {
        cleanup();
...
export const request = <T extends z.ZodSchema>(request: FirestoneRequest<T>, options?: { kill?: boolean }) => {
  return pipe(
    sendRequest(request, { sleep: 0 }),
    Effect.tap(() => Effect.logDebug('Waiting for response...')),
    Effect.flatMap(() => waitResponse(request.responseSchema, request.dataSchema, options)),
