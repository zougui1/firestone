...
export const findTargetStarFormation = async (
  ...
  if (highestWarMachineLevelWarMachine?.level) {
    for (const warMachine of currentWarMachinesInTeam) {
      ...
      const currentRarityLevel =
        warMachineRarityData.toLevel[warMachine.rarity];
      ...
      if (nexRarity) {
        ...
        if (warMachine.rarity === "common") {
          const highestRequiredLevel =
            warMachineRarityData.unlockLevel[
              highestWarMachineLevelWarMachine.rarity
            ];
          const highestRarityLevel =
            warMachineRarityData.toLevel[
              highestWarMachineLevelWarMachine.rarity
            ];
          const highestNextRarity =
            warMachineRarityData.fromLevel[highestRarityLevel + 1];

          if (highestNextRarity) {
            const highestNextRequiredLevel =
              warMachineRarityData.unlockLevel[highestNextRarity];
          ...
          const highestNextNextRarity =
            warMachineRarityData.fromLevel[highestRarityLevel + 2];

          if (highestNextRarity && highestNextNextRarity) {
            const highestRequiredLevel =
              warMachineRarityData.unlockLevel[highestNextRarity];
            const highestNextRequiredLevel =
              warMachineRarityData.unlockLevel[highestNextNextRarity];
            const difference = highestNextRequiredLevel - highestRequiredLevel;
  ...
  while (!canBeatTargetStar()) {
    ...
    const simulationWarMachines = computedData.warMachines
      ...
      .map((wm) => ({
        ...
        abilityActivationChance:
          warMachineRarityData.abilityActivationChance[wm.rarity],
      }));
    ...
    const currentRarityLevel =
      warMachineRarityData.toLevel[upgradeWarMachine.rarity];
    const nexRarity = warMachineRarityData.fromLevel[currentRarityLevel + 1];
