...
import { Dialog, Input, Label, Select } from "~/ui";
...
const NumberInput = ({ field, label, warMachineName }: NumberInputProps) => {
  const value = useSelector(
    gameDataStore,
    (state) => state.context.warMachines[warMachineName]?.[field],
  );
...
export const WarMachineFormDialog = ({
  warMachine,
  children,
}: WarMachineFormDialogProps) => {
  return null;
      ...
      <Dialog.Content>
        ...
        <div className="grid grid-cols-2 gap-4">
          <NumberInput
            label="Level"
            field="level"
            warMachineName={warMachine.name}
          />
          <NumberInput
            label="Sacred Card Level"
            field="sacredCardLevel"
            warMachineName={warMachine.name}
          />
          <NumberInput
            label="Lost Inscription Level"
            field="lostInscriptionLevel"
            warMachineName={warMachine.name}
          />
          <NumberInput
            label="Damage Blueprint Level"
            field="damageBlueprintLevel"
            warMachineName={warMachine.name}
          />
          <NumberInput
            label="Health Blueprint Level"
            field="healthBlueprintLevel"
            warMachineName={warMachine.name}
          />
          <NumberInput
            label="Armor Blueprint Level"
            field="armorBlueprintLevel"
            warMachineName={warMachine.name}
          />
