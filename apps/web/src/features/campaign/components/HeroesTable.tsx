...
import { cn, DataTable, Input } from "~/ui";
...
const getNumberInputColumn = (
  field: keyof CrewHero,
  label: string,
): ColumnDef<string> => {
  return {
    accessorKey: field,
    ...
    cell: function InputCell({ row }: CellContext<string, unknown>) {
      ...
      const value = useSelector(
        gameDataStore,
        (state) => state.context.crewHeroes[name]?.[field],
      );

      return (
        <Input value={value ?? ""} onChange={handleLevelChange(name, field)} />
      );
    },
  };
};
