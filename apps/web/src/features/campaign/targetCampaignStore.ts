...
import { defaultGameData, type WarMachine } from "./gameData";
...
export const targetCampaignStore = createStore({
  ...
  on: {
    ...
    changeMinimumSuccessChance: (
      context,
      event: { minimumSuccessChance: number },
    ) => {
      return produce(context, (draft) => {
    ...
    changeTargetFormation: (
      context,
      event: Pick<TargetCampaignState, "warMachines">,
    ) => {
      return produce(context, (draft) => {
...
targetCampaignStore
  .select((state) => state.minimumSuccessChance)
  .subscribe((minimumSuccessChance) => {
    optionalWindow?.localStorage.setItem(
