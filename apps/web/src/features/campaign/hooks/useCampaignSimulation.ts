...
export const simulateDetailedCampaign = async (
  ...
  await Promise.all(
    (Object.keys(data) as CampaignDifficulty[]).map(async (difficulty) => {
...
export const useCampaignSimulation = (
  options?: UseCampaignSimulationOptions,
) => {
  ...
  return useQuery({
    ...
    queryFn: async ({ signal }) => {
      ...
      const warMachines = data.warMachines
        .slice()
        .reverse()
        .map((wm) => ({
          ...wm,
