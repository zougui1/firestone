...
export const gameDataStore = createStore({
  context:
    parseStorageData(optionalWindow?.localStorage.getItem(storageKey) ?? "") ??
    defaultGameData,

  on: {
    updateWarMachine: (
      context,
      event: { name: string; data: Partial<Omit<WarMachine, "name">> },
    ) => {
    ...
    updateWarMachineRarity: (
      context,
      event: { name: string; rarity: WarMachineRarity },
    ) => {
      return produce(context, (draft) => {
        const warMachine = draft.warMachines[event.name];
    ...
    updateCrewHero: (
      context,
      event: { name: string; data: Partial<Omit<CrewHero, "name">> },
    ) => {
      return produce(context, (draft) => {
    ...
    updateArtifactTypes: (
      context,
      event: { name: string; data: Partial<ArtifactType["percents"]> },
    ) => {
      return produce(context, (draft) => {
