...
export const config = papr.model(
  "configs",
  schema({
    session: types.object(
      {
        id: types.string({ required: true }),
        status: types.enum(["valid", "invalid", "idle"] as const, {
          required: true,
        }),
    ...
    features: types.object(
      ...
      } satisfies Record<GameFeature, unknown>,
      { required: true },
    ),
  }),
);

export type Config = (typeof config)["schema"];
